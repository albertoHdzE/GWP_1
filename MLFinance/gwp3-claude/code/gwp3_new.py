...
class DataCollector:
    ...
    def fetch_data(self):
        ...
        self.data = yf.download(self.ticker, start=self.start_date, end=self.end_date, _fix_yahoo_issue=True)
...
class GAFEncoder:
    ...
    def encode_batch(self, time_series_matrix):
        ...
        for i in range(num_samples):
            ...
            # Resize to square output dimensions using bilinear interpolation
...
def plot_walk_forward_results(results_dict, metric='accuracy', title_suffix=''):
    ...
    Args:
        ...
        metric: Metric to plot ('accuracy' or 'returns')
...
def main():
    ...
    collector.save_data("spy_data.pkl")
    ...
    print(f"  - Mean daily return: {collector.data['returns'].mean():.6f}")
    print(f"  - Annualized return: {(1 + collector.data['returns'].mean())**252 - 1:.4f}")
    ...
    X_no_leakage, y_reg, y_class = preprocessor.create_features(collector.data, use_leakage=False)
    X_no_leakage_df = pd.DataFrame(X_no_leakage, columns=[f"Feature_{i}" for i in range(X_no_leakage.shape[1])], index=collector.data.index[n_lookback:])
    ...
    X_leakage, y_reg_l, y_class_l = preprocessor.create_features(collector.data, use_leakage=True)
    X_leakage_df = pd.DataFrame(X_leakage, columns=[f"Feature_{i}" for i in range(X_leakage.shape[1])], index=collector.data.index[n_lookback:])
    ...
    lstm = LSTMModel(time_steps=1, features=X_train.shape[1])  # Single time step
    print(f"  - Input shape: {lstm.input_shape}")
    
    print("\n2c. Building CNN-GAF model...")
    cnn_gaf = CNNGAFModel(image_size=128)
    print(f"  - Image size: {cnn_gaf.image_size}x{cnn_gaf.image_size}")
    ...
    results_no_leakage = run_step1(models_no_leakage, X_train, X_test,
    ...
    models_no_leakage_wf = {
        ...
        'LSTM': LSTMModel(time_steps=1, features=X_wl.shape[1]),
        'CNN-GAF': CNNGAFModel(image_size=128)
    }
    ...
    models_leakage = {
        ...
        'LSTM': LSTMModel(time_steps=1, features=X_train_l.shape[1]),
        'CNN-GAF': CNNGAFModel(image_size=128)
    }
    ...
    results_leakage = run_step1(models_leakage, X_train_l, X_test_l,
                                y_class_train_l, y_class_test_l,
                                leakage=True)
